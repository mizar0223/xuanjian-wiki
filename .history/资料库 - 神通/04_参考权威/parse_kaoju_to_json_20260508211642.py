...
def parse_quote_entries(lines, i):
    """解析原文引用条目列表，返回 (entries_list, new_i)"""
    entries = []
    while i < len(lines):
...
def parse_kaoju_md(filepath):
    ...
    while i < len(lines):
        ...
        # 匹配位置标题 - 两种格式
        # 格式1: #### 正位 - 正木 / #### 三阳一阴 - 少阳 / #### 附 - 青宣
        # 格式2: #### 清炁 / #### 玄雷 / #### 全丹 （无位置类型前缀）
        weizhi_match = re.match(r'^#### (.+)', line)
        if weizhi_match and current_tixi is not None:
            full_title = weizhi_match.group(1).strip()
            
            # 判断是否包含 " - " 分隔符
            if ' - ' in full_title:
                parts = full_title.split(' - ', 1)
                weizhi_type = parts[0].strip()
                weizhi_name = parts[1].strip()
            else:
                # 无位置类型前缀的格式（十二炁、三雷、三巫、二祝、并古、独立等）
                weizhi_type = "—"
                weizhi_name = full_title
            ...
            if i < len(lines) and '道统' in lines[i] and '原文出处' in lines[i]:
                ...
                i += 1
                # 解析原文条目
                entries, i = parse_quote_entries(lines, i)
                current_weizhi["道统原文出处"] = entries
                continue
            elif i < len(lines) and '原文出处' in lines[i] and '道统' not in lines[i]:
                # 有些格式是 **道统「XXX」原文出处** 但不含"道统"二字
                header_line = lines[i]
                match_info = re.search(r'共 (\d+) 处', header_line)
                display_info = re.search(r'展示 (\d+) 处', header_line)
                total_matches = int(match_info.group(1)) if match_info else 0
                display_count = int(display_info.group(1)) if display_info else total_matches
        ...
        if shentong_match and current_weizhi is not None:
            ...
            if i < len(lines):
                ...
                if info_line.startswith('>'):
                    info_line = info_line.lstrip('> ').strip()
                ...
                if match_info:
                    ...
                    if '全量保留' in info_line:
                        ...
                        if sample_match:
                            ...
                            sample_match2 = re.search(r'展示 (\d+) 处', info_line)
                            if sample_match2:
                                current_shentong["保留策略"] = f"智能抽样({sample_match2.group(1)}/{match_info.group(1)})"
                            else:
...
def main():
    ...
    for t in result["体系列表"]:
        w_count = len(t["位置列表"])
        s_count = sum(len(w["神通列表"]) for w in t["位置列表"])
        print(f"    【{t['体系名称']}】: {w_count}个道统, {s_count}个神通")
