...
上传脚本：`/data/workspace/rq0rlzeg/xuanjian-wiki/scripts/upload_pages.py`
凭证：`/data/workspace/rq0rlzeg/xuanjian-wiki/.env`（WIKI_USER, WIKI_PASS）
...
```bash
cd /data/workspace/rq0rlzeg/xuanjian-wiki
...
| 优先级 | 事项 | 量级 | 状态 |
|--------|------|------|------|
| P1 | 中频遗漏实体 Batch 1（10个）评估/补录 | 10个高优先级 | 可行性高 | 紧迫 | ⏳ 之前待决策（评估报告已出） | 相互依赖 |
| P1 | 中频遗漏实体 Batch 2（7个）+ Batch 3（2个）评估/补录 | 9个 | ✅ 2026-05-12 完成 |
...
| — | ~~中频遗漏实体 Batch 1 补录+审计~~ | 10个 | ✅ 2026-05-12 完成 |
| — | ~~中频遗漏实体 Batch 2 + Batch 3 补录~~ | 9个 | ✅ 2026-05-12 完成（按v3.1 workflow） |
| — | ~~中频遗漏实体全部完成~~ | 21个评估 → 10+7+2=19补录+2排除 | ✅ 全部闭环 |
...
| 场景 | 命令 |
|---|---|
| 上传所有 pages | `cd ../.. && set -a && source .env && set +a && python3 scripts/upload_pages.py --only "资料库/造物/pages" --summary "xxx"` |
